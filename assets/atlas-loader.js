...
async function load(path){
  ...
  if(path==='assets/partner-export-template.json')return (await fetchChecked(path+'?v=20260917-6')).arrayBuffer();
  if(path==='data/funding-review-20260917.json')return (await fetchChecked(path)).arrayBuffer();
  if(path==='donors.html'){
    const page=await (await fetchChecked('pages/donors-20260917.json?v=20260917-6')).json();
