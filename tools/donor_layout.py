...
def refine(html, root):
    ...
    def control(key):
        ...
        return matches[0].replace('County relevance<select','Geography<select')
...
<ol><li><strong>Set your focus</strong><p>Choose a sector and geography. Open More filters to search for a project or organisation.</p></li>
...
<li><strong>Build your funding case</strong><p>Review <b>Project evidence</b> for partners and possible duplication. Use <a href="#analysis" target="_parent">Country analysis</a> to support your assessment of need.</p></li></ol>
    ...
    primary='<div class="filterbar primary-filters" aria-label="Focus your search">'+''.join(control(k) for k in ['sector','county'])+'</div>'
    advanced='''<div class="filter-options"><details id="advancedFilters" class="advanced-filters"><summary>More filters <span class="filter-hint">Search, partner, years and finance</span><span id="advancedCount" class="filter-count" hidden></span></summary><div class="advanced-grid">'''+''.join(control(k) for k in ['search','donor','from','to','measure','instrument','coverage'])+'''</div></details><button id="reset" class="reset-filters">Reset filters</button></div>'''
